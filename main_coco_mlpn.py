...
options:
    ...
    --savepath=savepath           Savepath [default: ./save/coco_trans]
...
valset = CocoSet(args['datapath'], 'train_eval', args)
...
for epoch in range(1, 1000):
    ...
    for jj, batch in enumerate(train_loader, 0):
        ...
        # #set preds
        # sample_sz = label_support.shape[0]
        # sample_dist = dist_all[:sample_sz, :sample_sz]
        # alpha_L = sample_dist.mm(label_support_norm) # S x L
        #
        # inv_aul = alpha_L.transpose(0, 1).mm(alpha_L).double()
        # inv_aul = inv_aul + torch.eye(inv_aul.shape[0], inv_aul.shape[0]).cuda().double()*1e-7
        # inv_aul = inv_aul.inverse()
        #
        # label_support_minusone = label_support*2. - 1
        # P = label_support_minusone.transpose(0, 1).mm(alpha_L).mm(inv_aul)



        # #protoloss
        # inv_aul = label_support_norm.transpose(0, 1).mm(label_support_norm).double()
        # inv_aul = inv_aul + torch.eye(inv_aul.shape[0], inv_aul.shape[0]).cuda().double()*1e-7
        # inv_aul = inv_aul.inverse()
        # A_uu_label_q = A_uu.mm(label_queries_norm.double())
        # A_ul_pred = -A_uu_label_q.mm(inv_aul).detach()
        #
        # A_ul_pred = A_ul_pred.unsqueeze(-1).repeat(1, 1, 1600).float()
        # mixture_samples = A_ul_pred*support_samples.unsqueeze(0).repeat(query_size, 1, 1)
        # mixture_samples = mixture_samples.sum(1)
        # protoloss = torch.mean((query_features - mixture_samples))


        all_support_features = support_samples
        ...
        ######################### LOSSSSSSS #########################################################################
        loss = F.mse_loss(logits.float(), label_queries_norm) + 0.01*loss_count#+ protoloss
        ...
        #acc = accuracy_calc(logits.float(), label_queries.float(), label_size=total_label_size, topk=3, greaterthan=0.1)
        # print('[ACC] loss= {:.4f} op={:.4f} or={:.4f} F1={:.4f}'
        #       .format(loss, acc[0], acc[1], acc[2]))
        va_PR.add(acc[0])
        va_RE.add(acc[1])
        va_F1.add(acc[2])
        lossva.add(loss)
